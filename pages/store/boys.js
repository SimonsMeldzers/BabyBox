...
function Boys({featuredProducts}) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const featuredQuery = `*[_type == "product" && gender == "boy"] | order(${sortOrder})`;
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <StoreBanner
        title="Preces Zēniem"
        breadcrumbLink="/store/boys"
        breadcrumbName="Zēniem"
      />
...
export const getServerSideProps = async () => {
  const featuredQuery = `*[_type == "product" && gender == "boy"]`;
